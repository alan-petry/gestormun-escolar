...
export default function V_matriculasList () {
    const context = useContext(GlobalContext);
    const history = useHistory();
    const [lista, setLista] = useState<V_matriculasProps[]>([]);
    const [filtro, setFiltro] = useState<V_matriculasProps>({MAT_CODIGO : 0, ALU_NOME : ""})
  ...
  function showDialog(props: DialogProps){
    ...
    async function getLista(){
      if (filtro.ALU_NOME) {
        ...
        if (aux.length >= 3){
          try {
              const response = await api.post('/cadastros/v_matriculas_listar', {ALU_NOME: filtro.ALU_NOME});
    ...
    function handleSelect(id: number){
      history.push(`/admin/v_matriculas_edit/${id}`);
    }

    async function handleDelete(id?: number){
      try {
          console.log(`delete ${id}`);
          await api.delete(`/cadastros/v_matriculas/${id}`);
          getLista();
    ...
    <div className="content">
      ...
      <Row>
        <Col lg="12" md="12" sm="12">
          <Card>
            <CardHeader>
              <CardTitle tag="h5">Consulta de V_matriculas</CardTitle>
              ...
              {'  '}<Link to={'/admin/v_matriculas_new'} className="btn btn-primary">Novo</Link>
            </CardHeader>
            <CardBody>
              {lista.length > 0 ?
                <>
