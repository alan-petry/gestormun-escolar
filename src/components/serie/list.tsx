...
export default function SerieList () {
    ...
    const [filtro, setFiltro] = useState<SerieProps>({SER_CODIGO : 0, SER_DESC : ""})
  ...
  function showDialog(props: DialogProps){
    ...
    async function getLista(){
      if (filtro.SER_DESC) {
        let aux = filtro.SER_DESC;
        if (aux.length >= 3){
          try {
              const response = await api.post('/cadastros/serie_listar', {SER_DESC: filtro.SER_DESC});
    ...
    function handleSelect(id: number){
      history.push(`/admin/serie_edit/${id}`);
    }

    async function handleDelete(id?: number){
      try {
          console.log(`delete ${id}`);
          await api.delete(`/cadastros/serie/${id}`);
          getLista();
    ...
    <div className="content">
      ...
      <Row>
        <Col lg="12" md="12" sm="12">
          <Card>
            <CardHeader>
              <CardTitle tag="h5">Consulta de Serie</CardTitle>
              <Input className='form-control' name="SER_DESC"
                placeholder="Informe algo para consultar"
                onChange={event => setFiltro({ ...filtro, SER_DESC: event.target.value })}></Input>
              <Button className="btn btn-success" onClick={getLista}>Filtrar dados</Button>
              {'  '}<Link to={'/admin/serie_new'} className="btn btn-primary">Novo</Link>
            </CardHeader>
            <CardBody>
              {lista.length > 0 ?
                <>
                      ...
                      <tr>
                        ...
                        <td className="text-right">
                          <Button
                            ...
                            size="sm"
                            type="button"
                            onClick={() => handleSelect(item.SER_CODIGO)} 
                          >
                          ...
                          <Button
                            ...
                            onClick={() => showDialog({
                              type:'confirmation',
                              msg:'Deseja excluir o registro: ' + item.SER_DESC,
                              title: 'Confirmacao',
