...
export default function AlunoList () {
    ...
    const [filtro, setFiltro] = useState<AlunoProps>({ALU_CODIGO : 0, ALU_NOME : ""})
  ...
  function showDialog(props: DialogProps){
    ...
    async function getLista(){
      if (filtro.ALU_NOME) {
        ...
        if (aux.length >= 3){
          const response = await api.post('/cadastros/aluno_listar', {ALU_NOME: filtro.ALU_NOME});
    ...
    function handleSelect(id: number){
      history.push(`/admin/aluno_edit/${id}`);
    }

    async function handleDelete(id?: number){
      try {
          console.log(`delete ${id}`);
          await api.delete(`/cadastros/aluno/${id}`);
          getLista();
    ...
    <div className="content">
      ...
      <Row>
        <Col lg="12" md="12" sm="12">
          <Card>
            <CardHeader>
              <CardTitle tag="h5">Consulta de Aluno</CardTitle>
              ...
              {'  '}<Link to={'/admin/aluno_new'} className="btn btn-primary">Novo</Link>
            </CardHeader>
            <CardBody>
              {lista.length > 0 ?
                <>
                      ...
                      <tr>
                        ...
                        <td className="text-right">
                          <Button
                            ...
                            size="sm"
                            type="button"
                            onClick={() => handleSelect(item.ALU_CODIGO)} 
                          >
